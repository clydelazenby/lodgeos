 ...
 * expired, with no password reset flow anywhere in the app to recover
 * with. app/auth/callback verifies his token and sends him here first.
 */
export default function SetPasswordPage() {
  ...
  useEffect(() => {
    createClient().auth.getUser().then(({ data }) => {
      if (!data.user) {
        window.location.replace('/auth/login?error=link_expired')
        return
      }
      setChecking(false)
    })
    ...
    <div style={{ minHeight: '100vh', background: '#0A0E1A', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      <div style={{ width: '100%', maxWidth: '460px' }}>
        <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
          ...
          <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.6rem', color: '#B8B0A0', letterSpacing: '0.2em', marginTop: '4px' }}>SET UP YOUR PORTAL ACCESS</p>
        </div>
        ...
        <div style={{ background: '#141C2E', border: '1px solid rgba(201,168,76,0.2)', padding: '2.5rem', borderRadius: '8px' }}>
          {checking ? (
            ...
            <>
              <h1 style={{ fontFamily: 'Cinzel, serif', fontSize: '1.2rem', color: '#F5F0E8', marginBottom: '0.4rem' }}>Choose a password</h1>
